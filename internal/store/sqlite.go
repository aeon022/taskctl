...
func (s *Store) UpsertTask(ctx context.Context, t *models.Task) error {
	...
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO tasks (id,title,list,notes,status,due_date,priority,external_id,source,created_at,updated_at,completed_at)
		VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
	...
	`,
		t.ID, t.Title, t.List, t.Notes, t.Status, due, t.Priority,
...
type ListFilter struct {
	List   string // "" = all lists
	Status string // "" = all, "needsAction", "completed"
}

func (s *Store) ListTasks(ctx context.Context, f ListFilter) ([]models.Task, error) {
	query := `SELECT id,title,list,notes,status,due_date,priority,external_id,source,created_at,updated_at,completed_at FROM tasks WHERE 1=1`
	var args []any
	if f.List != "" {
		query += ` AND list = ?`
